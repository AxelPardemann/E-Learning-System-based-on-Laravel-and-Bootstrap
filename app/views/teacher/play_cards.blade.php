  ...
  <div class="mainpanel">
    ...
    <div class="contentpanel">      
      <div class="row">
        ...
        {{Form::open(array('url' => 'teacher_quizcomplete', 'id' => 'quizform', 'method' => 'post'))}}
        ...
        <div class="flow-cards col-md-6">
          <div class="show card-0 current">
            <div class="card-number">Show Card 1</div>
            <div class="front card-table {{$cards[0]->card_type}}">
              <div class="listen">
                @if ($cards[0]->f_sound_option == 1)
                  <span class="fa fa-volume-up"></span><br />
                @else
                  <span class="no-sound">&nbsp;</span><br />
                @endif
                @if ($cards[0]->f_text_option == 1)
                  <span class="fa">{{$cards[0]->f_text}}</span><br />
                @endif
                <div class="interval_bar">
                  <div class="real_bar"></div>
                </div>
              ...
              <div class="prompt">
                @if ($cards[0]->card_type == "singlecard")
                  <input type="text" class="answer">
                  <input type="button" class="answer-button singlecard" value="Answer" />
                  <div class="show-answer">Correct Answer</div>
                  <input type="text" class="correct" value="{{$cards[0]->b_text}}" disabled />
                @elseif ($cards[0]->card_type == "radiocard")
                  <ul>
                  @foreach ($subcards as $subcard)
                    @if ($subcard->cards == $cards[0]->id && $subcard->correctanswer == 1)
                      <div class="show-answer">Correct Answer</div>
                    @endif
                    @if ($subcard->cards == $cards[0]->id)
                      <li>
                        <div class="rdio rdio-default">
                          <input type="radio" name="sub-0" id="radio_0_{{$subcard->id}}" value="{{$subcard->answer}}" />
                          <label for="radio_0_{{$subcard->id}}">{{$subcard->answer}}</label>
                        </div>                      
                      </li>
                    @endif
                  @endforeach
                  </ul>
                  <input type="hidden" name="option-0" class="cardoption" id="option-0">
                  <input type="button" class="answer-button radiocard" value="Answer" />
                @elseif ($cards[0]->card_type == "checkcard")
                  <ul>
                  @foreach ($subcards as $subcard)
                    @if ($subcard->cards == $cards[0]->id && $subcard->correctanswer == 1)
                      <div class="show-answer">Correct Answer</div>
            ...
            <div class="back card-table">
              @if ($cards[0]->card_type == "singlecard")
                @if ($cards[0]->b_text_option == 1)
                  <span class="fa">{{$cards[0]->b_text}}</span>
                @endif
              @elseif ($cards[0]->card_type == "radiocard" || $cards[0]->card_type == "checkcard")
                @foreach ($subcards as $subcard)
                  @if ($subcard->cards == $cards[0]->id && $subcard->correctanswer == 1)
                    @if ($subcard->b_text_option == 1)
                      <span class="fa">{{$subcard->answer}}</span>
                    @endif
                    @if ($cards[0]->card_type == "checkcard")
                      <span class="fa">,</span>
                    @endif
                  @endif
                @endforeach
          ...
          <div class="card-1">
            <input type="hidden" class="real-id" value="{{$cards[0]->id}}" />
            <div class="card-number">Test Card 1</div>
            <div class="front card-table {{$cards[0]->card_type}}">
              <div class="listen">
                @if ($cards[0]->f_sound_option == 1)
                  <span class="fa fa-volume-up"></span><br />
                ...
                @elseif ($cards[0]->card_type == "radiocard")
                  ...
                  @foreach ($subcards as $subcard)
                    ...
                    @if ($subcard->cards == $cards[0]->id)
                      <li>
                        <div class="rdio rdio-default">
                          <input type="radio" name="sub-1" id="radio_{{$subcard->id}}_1" value="{{$subcard->answer}}" />
                          <label for="radio_{{$subcard->id}}_1">{{$subcard->answer}}</label>
                        </div>                          
                      </li>
                    @endif
                  @endforeach
                  </ul>
                  <input type="hidden" name="option-1" class="cardoption" id="option-1">
                  <input type="button" class="answer-button radiocard" value="Answer" />
                @elseif ($cards[0]->card_type == "checkcard")
                  <ul>
                  @foreach ($subcards as $subcard)
...
jQuery(document).ready(function() {
  ...
  setTimeout(function() {
    $('#play-1').find('.simpleplayer-play-control').click();
    ...
    if (!show_flag) {
      if (card_flag) {
        ...
        if (correct_count < Math.min(active, $('.hidden-cards>div').length / 2)) {
          ...
          appendTxt += $('.hidden-cards .show-' + correct_count).html();
          appendTxt += '</div>';
          ...
          appendTxt += $('.hidden-cards .test-' + correct_count).html();
          appendTxt += '</div>';
                      ...
                      j = 0;
                       ...
                       if (real_id[unique_cards[j]] == k) {
                        ...
                        if (unique_cards.length == 1) { 
                          ...
                          //j = Math.floor(Math.random() * unique_cards.length);
                          if (j == 0){
                            j ++;
